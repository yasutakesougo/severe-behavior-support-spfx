...
test('frozen 10-path sbs-domain bundle baseline is literal and non-expanding', () => {
  ...
  assert.throws(() => BASELINE_BUNDLE_PATHS.push('spfx/src/sbs-domain/new.bundle.js'), TypeError);
...
test('name-status parser consumes R100 and C100 as old + resulting path pairs', () => {
  const parsed = parseNameStatusZ(nulTokens(
    'R100', 'old/path.txt', 'new/path.txt',
    'C100', 'source/path.txt', 'copy/path.txt',
  ));
...
test('DELETE has no resulting path and therefore cannot trigger resulting-path policy by itself', () => {
...
test('malformed or truncated NUL streams fail closed', () => {
  assert.throws(() => parseNameStatusZ(Buffer.from('A\0path-without-final-nul', 'utf8')), /TRUNCATED_NUL_STREAM/u);
  assert.throws(() => parseNameStatusZ(nulTokens('R100', 'only-old-path')), /MALFORMED_RENAME_COPY_STATUS/u);
});
...
test('ls-tree parser rejects zero, multiple, malformed, and non-blob records', () => {
  ...
  assert.throws(
    () => parseLsTreeZ(nulTokens(
      `100644 blob ${'a'.repeat(40)}\ta.txt`,
      `100644 blob ${'b'.repeat(40)}\tb.txt`,
    )),
    /LS_TREE_MULTIPLE_RECORDS/u,
  );
  assert.throws(() => parseLsTreeZ(nulTokens('not-ls-tree')), /LS_TREE_MALFORMED_RECORD/u);
