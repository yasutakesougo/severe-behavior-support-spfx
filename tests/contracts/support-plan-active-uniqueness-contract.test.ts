...
describe("Active plan uniqueness contract (Issue #24)", () => {
  ...
  it("Active 1件 → UNIQUE", () => {
    assert.equal(
      evaluateActivePlanUniqueness([
        active("synthetic-plan-a", "2026-08-06T14:00:00.000Z"),
      ]),
      "UNIQUE",
    );
  });
  ...
  it("複数判定単位を含み、1単位だけ CONFLICT → 配列全体 CONFLICT", () => {
    assert.equal(
      evaluateActivePlanUniqueness([
        active(
          "synthetic-plan-user-a-1",
          "2026-01-01T00:00:00.000Z",
          "2026-01-31T00:00:00.000Z",
          { UserId: "synthetic-user-a" },
        ),
        active(
          "synthetic-plan-user-a-2",
          "2026-01-15T00:00:00.000Z",
          "2026-02-15T00:00:00.000Z",
          { UserId: "synthetic-user-a" },
        ),
        active(
          "synthetic-plan-user-b-1",
          "2026-01-01T00:00:00.000Z",
          "2026-01-31T00:00:00.000Z",
  ...
  it("UserIdが同じでもSiteIdが異なる → 相互に競合させない", () => {
    assert.equal(
      evaluateActivePlanUniqueness([
        active(
          "synthetic-plan-site-a",
          "2026-04-01T00:00:00.000Z",
          "2026-04-30T00:00:00.000Z",
          { SiteId: "synthetic-site-a" },
        ),
        active(
          "synthetic-plan-site-b",
          "2026-04-01T00:00:00.000Z",
          "2026-04-30T00:00:00.000Z",
  ...
  it("SiteIdが同じでもOrganizationIdが異なる → 相互に競合させない", () => {
    assert.equal(
      evaluateActivePlanUniqueness([
        active(
          "synthetic-plan-org-a",
          "2026-04-01T00:00:00.000Z",
          "2026-04-30T00:00:00.000Z",
          { OrganizationId: "synthetic-org-a" },
        ),
        active(
          "synthetic-plan-org-b",
          "2026-04-01T00:00:00.000Z",
          "2026-04-30T00:00:00.000Z",
  ...
  it("不正status等の壊れた入力をUNIQUEへ倒さない", () => {
    ...
    assert.equal(
      evaluateActivePlanUniquenessUnknown("not-an-array"),
      "MALFORMED_INPUT",
    );
  });
  ...
  it("UTC瞬間が前日でも Asia/Tokyo 暦日へ変換する", () => {
    ...
    assert.equal(
      toAsiaTokyoCalendarDay("2026-08-05T16:00:00.000Z"),
      "2026-08-06",
    );
  });
});
