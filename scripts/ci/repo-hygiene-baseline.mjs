...
export const SBS_DOMAIN_BUNDLE_PATTERNS = Object.freeze([
  ...SBS_DOMAIN_BUNDLE_PATTERNS_LITERAL,
]);
...
function assertConfiguredPattern(pattern) {
  ...
  if (!pattern.includes("/")) {
    throw new Error(
      `repo-hygiene bare-basename pattern is forbidden: ${JSON.stringify(pattern)}`,
    );
  }
  if (pattern.startsWith("/") || pattern.includes("\\") || pattern.includes("\0")) {
    throw new Error(
      `repo-hygiene pattern is not repository-relative POSIX form: ${JSON.stringify(pattern)}`,
    );
  }
}
...
const COMPILED_BLOCKED_PATTERNS = Object.freeze(
  BLOCKED_PATTERNS.map((pattern) =>
    Object.freeze({ pattern, regex: compilePolicyPattern(pattern) }),
  ),
);
const COMPILED_BUNDLE_PATTERNS = Object.freeze(
  SBS_DOMAIN_BUNDLE_PATTERNS.map((pattern) =>
    Object.freeze({ pattern, regex: compilePolicyPattern(pattern) }),
  ),
);
...
if (BASELINE_BUNDLE_PATHS.length !== 10 || new Set(BASELINE_BUNDLE_PATHS).size !== 10) {
  throw new Error(
    "repo-hygiene frozen sbs-domain bundle baseline must contain exactly 10 unique literal paths",
  );
}
