...
export function parseNameStatusZ(buffer) {
  ...
  for (let index = 0; index < tokens.length; ) {
    ...
    if (kind === "A" || kind === "M" || kind === "D") {
      if (status.length !== 1 || index >= tokens.length) {
        guardError("MALFORMED_NAME_STATUS");
      }
    ...
    if (kind === "R" || kind === "C") {
      ...
      index += 2;
      changes.push(
        Object.freeze({ status, kind, oldPath, resultingPath, deletedPath: null }),
      );
      continue;
    }
...
export function parseLsTreeZ(buffer) {
  ...
  if (records.length !== 1) {
    guardError(records.length === 0 ? "LS_TREE_ZERO_RECORDS" : "LS_TREE_MULTIPLE_RECORDS");
  }
...
function runGit(args, options = {}) {
  ...
  if (result.status !== 0 && !options.allowFailure) {
    guardError(options.errorCode ?? "GIT_COMMAND_FAILED");
  }
  return result;
}
...
function assertNoPersistedAuth(repository) {
  ...
  if (extraHeader.status !== 0 && extraHeader.status !== 1) {
    guardError("AUTH_CONFIG_CHECK_ERROR");
  }
  ...
  if (credentialHelper.status !== 0 && credentialHelper.status !== 1) {
    guardError("AUTH_CONFIG_CHECK_ERROR");
  }
}
...
function fetchPullRequestHead({ prNumber, expectedHeadSha, repository, token }) {
  ...
  if (typeof token !== "string" || token.length === 0 || /[\r\n]/u.test(token)) {
    guardError("MISSING_GITHUB_TOKEN");
  }
...
function resolveResultingBlob(diffHead, path) {
  ...
  if (!record.pathBytes.equals(requestedBytes) || record.path !== path) {
    guardError("LS_TREE_PATH_MISMATCH");
  }
  if (record.type !== "blob") guardError("LS_TREE_NON_BLOB");

  const sizeRaw = gitStdout(["cat-file", "-s", record.sha], "CAT_FILE_SIZE_ERROR")
    .toString("ascii")
    .trim();
...
function loadContext() {
  ...
  const policySourceSha = requireSha(
    process.env.RH_POLICY_SOURCE_SHA ?? "",
    "INVALID_POLICY_SOURCE_SHA",
  );
  ...
  if (eventName === "pull_request_target") {
    ...
    const prNumber = requirePositiveDecimal(
      process.env.RH_PR_NUMBER ?? "",
      "INVALID_PR_NUMBER",
    );
...
function emitPackObservation(kib) {
  ...
  if (kib > 25 * 1024) {
    ...
    console.log(
      `REPO_HYGIENE_PACK observation=PASS_INFORMATIONAL sizeMiB=${mib.toFixed(2)}`,
    );
  }
}
...
export async function main() {
  ...
  console.log(
    `REPO_HYGIENE_RESULT verdict=PASS findings=0 baselineBundlePaths=${BASELINE_BUNDLE_PATHS.length}`,
  );
  process.exitCode = 0;
}
