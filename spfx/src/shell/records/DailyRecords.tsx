...
export const DailyRecords: React.FC<DailyRecordsProps> = ({ presentation, headingRef }) => {
  const { heading, inputPersonLabel, inputPrompt, incompleteItems, recentRecords, businessFacts, systemState } =
    presentation;

  return (
    <section className={styles.dailyRecords} data-demo-ux="daily-records" aria-labelledby="demo-ux-records-heading">
      <p className={styles.presentationNote} data-demo-ux="daily-record-presentation-note">
        {DEMO_DAILY_RECORD_PRESENTATION_NOTE}
      </p>
      <h1 id="demo-ux-records-heading" ref={headingRef} tabIndex={-1} data-demo-ux="daily-record-heading">
        {heading}
      </h1>
      ...
      <section className={styles.section} aria-labelledby="demo-ux-record-input-heading">
        ...
        <div className={styles.inputGrid}>
          ...
          <label className={styles.fullWidth}>
            記録内容
            <textarea defaultValue="合成表示のため入力できません。" disabled aria-disabled="true" rows={4} />
          </label>
        </div>
        <p className={styles.mutationNote}>{DEMO_DAILY_RECORD_MUTATION_DISABLED_NOTE}</p>
        <div className={styles.actionRow}>
          <button type="button" disabled aria-disabled="true">作成する</button>
          <button type="button" disabled aria-disabled="true">保存する</button>
        </div>
      </section>
      ...
      <div className={styles.stateGrid}>
        <section className={styles.statePanel} aria-labelledby="demo-ux-record-business-heading">
          ...
          <dl>
            <div><dt>記録対象</dt><dd>{businessFacts.recordScopeLabel}</dd></div>
            <div><dt>担当</dt><dd>{businessFacts.responsibleRoleLabel}</dd></div>
          </dl>
        </section>
        <section className={styles.statePanel} aria-labelledby="demo-ux-record-system-heading">
          ...
          <dl>
            <div><dt>保存状態</dt><dd>{systemState.saveStateLabel}</dd></div>
            <div><dt>データ元</dt><dd>{systemState.dataSourceLabel}</dd></div>
          </dl>
        </section>
      </div>
    </section>
