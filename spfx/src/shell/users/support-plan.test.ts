...
describe("SUPPORT-PLAN-REVIEW-NEW-VERSION-DEMO-1", () => {
  ...
  it("presents next-version concept without persistence or final-approval meaning", () => {
    ...
    expect(SUPPORT_PLAN_DRAFT_ACTIVE_LABEL).toBe("適用中");
    ...
    expect(SUPPORT_PLAN_AFTER_APPLY_CURRENT_REMAINS_NOTE).toBe("現在の版はそのまま残ります。");
    expect(SUPPORT_PLAN_AFTER_APPLY_HISTORY_PREFIX).toBe("過去版");
    expect(SUPPORT_PLAN_ACTIVATION_INFO_HEADING).toBe("適用情報");
    expect(
      supportPlanCopyAvoidsFinalApprovalMeaning(SUPPORT_PLAN_AFTER_APPLY_NEXT_CHANGE_NOTE),
    ).toBe(true);
    expect(
      supportPlanCopyAvoidsFinalApprovalMeaning(SUPPORT_PLAN_AFTER_APPLY_CURRENT_REMAINS_NOTE),
    ).toBe(true);
  });
});
