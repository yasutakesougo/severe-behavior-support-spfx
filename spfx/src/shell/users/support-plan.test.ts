...
describe("SUPPORT-PLAN-REVIEW-NEW-VERSION-DEMO-1", () => {
  ...
  it("presents next-version concept without persistence or final-approval meaning", () => {
    ...
    expect(SUPPORT_PLAN_AFTER_APPLY_NEXT_VERSION_NOTE).toBe(
      "次に変更するときは、新しい版を作ります。",
    );
    expect(SUPPORT_PLAN_AFTER_APPLY_BOUNDARY).toBe("本番未保存");
    expect(SUPPORT_PLAN_ACTIVATION_INFO_HEADING).toBe("適用情報");
    expect(
      supportPlanCopyAvoidsFinalApprovalMeaning(SUPPORT_PLAN_AFTER_APPLY_NEXT_VERSION_NOTE),
    ).toBe(true);
  });
});
