...
export const DEMO_KPI_FAMILY_P_NOTE =
  "要確認・見直し時期・観察待ちは、この画面の支援計画一覧と同じ利用者を対象にしています。利用者一覧の要確認・未記録・期限接近とは対象が異なります。";
...
export const SUPPORT_PLAN_MANAGEMENT_VERSION_NONE_LABEL = "現在の版: なし" as const;
...
export const SUPPORT_PLAN_MANAGEMENT_EMPTY_NOTE = "表示できる支援計画はありません。";

export const SUPPORT_PLAN_MANAGEMENT_TODAY_EMPTY_NOTE = "今日やることはありません。";
...
export function formatSupportPlanManagementObservationLabel(dateLabel: string | undefined): string {
  ...
  return `最終観察日: ${formatStaffBusinessDate(dateLabel)}`;
}
