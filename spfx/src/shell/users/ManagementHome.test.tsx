...
describe("SBS-MGMT-HOME-C ManagementHome", () => {
  it("renders the staff-facing hierarchy from the resolved snapshot", () => {
    const html = renderToStaticMarkup(<ManagementHome model={MANAGEMENT_HOME_FULLY_RESOLVED_MODEL} />);
  ...
  it("renders confirmed none without calling it unavailable", () => {
    const html = renderToStaticMarkup(<ManagementHome model={MANAGEMENT_HOME_CONFIRMED_NONE_MODEL} />);
