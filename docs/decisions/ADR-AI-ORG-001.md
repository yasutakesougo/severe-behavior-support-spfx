...
- 承認範囲の上限:
  ...
  - ディレクトリ新設は承認しない
  - 既存正本移動は承認しない
  - `docs/process/ai-*.md` や `.agents/agents|commands|mcp/` の作成は承認しない
  - DEC-AI-ORG-3 の内容は承認しない
...
- 計画正本の「物理配置」判断単位を閉じる
- `DEC-AI-ORG-3`は、`.agents/mcp/`を将来の権限マトリクス実体候補として扱ってよい（作成・配置の実行は別承認）
...
本ADRは`承認`済みである。次を個別に作成・承認する。

```text
DEC-AI-ORG-3
```

後続文書の作成は本ADRの配置案を前提とするが、DEC-AI-ORG-3の内容承認および物理変更の実行は別途必要である。
