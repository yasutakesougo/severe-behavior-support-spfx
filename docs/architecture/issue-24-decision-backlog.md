...
Decision-SEV-2-VOCAB: HOLD（V-C / NOT DEFINED / 再評価待ち）
Decision-SEV-2-ASSIGN: CANDIDATE / NOT SELECTED
...
| 正式値・意味 | SEV-2-VOCAB HOLD / V-C | **NOT DEFINED**（再評価待ち。`"10+"`/`"18+"` 直写禁止。暗黙値禁止） |
| domain 算出 vs caller-supplied | SEV-2-ASSIGN（CANDIDATE / NOT SELECTED） | **未決**（VOCAB Accepted 後の別判断） |
...
6. Decision-SEV-2   FindingSeverity boundary — PURPOSE **RECORDED**；CONCEPT-INV **COMPLETED**；VOCAB **HOLD / V-C**（再評価待ち）；ASSIGN は CANDIDATE / NOT SELECTED
...
SEV は SEV-1 → SEV-2 packet。PURPOSE は MHLW-first。CONCEPT-INV は COMPLETED。次は Human VOCAB 再評価。ASSIGN は独立 CANDIDATE / NOT SELECTED。一括 Accepted しない。FC は FC-1→FC-2 の順を崩さない。
...
Decision-SEV-2-CONCEPT-INV: COMPLETED / OFFICIAL_CONCEPT_EXISTS
Decision-SEV-2-VOCAB: HOLD（V-C / NOT DEFINED / 再評価待ち）
Decision-SEV-2-ASSIGN: CANDIDATE / NOT SELECTED（Implementation NOT STARTED）
Issue #24 Close: NO-GO
...
```text
1. SEV-2-CONCEPT-INV 結果（行動関連項目合計点数 / 閾値10・18 / 汎用 Severity NOT FOUND）を正本へ記録する
2. SEV-2-VOCAB は HOLD 維持し、Human 再評価候補 Option A/B を提示するだけとする
