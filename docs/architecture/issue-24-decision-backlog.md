...
| **Decision-FC-3** | Catalog snapshot logical contract | **Accepted / Option C**。Complete logical contract surface（必須情報・version 一意識別責務・immutable・selected 整合・fail-closed）。正本: [`decision-fc-3-finding-code-catalog-snapshot-logical-contract.md`](./decision-fc-3-finding-code-catalog-snapshot-logical-contract.md) | FC-2 Accepted（Option C） | version 文字列・schema・storage・provider・値一覧は決めない | materialization / schema / provider は別 Human Start。Implementation HOLD |
...
**Decision-FC-1 = Accepted / Option B**。**Decision-FC-2 = Accepted / Option C**。**Decision-FC-3 = Accepted / Option C**。値一覧・enum・validator・version 文字列・storage / provider は未決。Issue #8 FindingCode DEC 番号は UNASSIGNED。
...
Dependency blocker（#29 mapping）: CLEARED
Concrete Repository Entry Review: PASS（Issue #22 comment 5224544473）
Next: SEV-2-VOCAB Canonical COMPLETE（DEC-018）。FC-1 Accepted / Option B。FC-2 Accepted / Option C。FC-3 Accepted / Option C。FindingCode 値一覧未決。実 SharePoint adapter は別 Gate / NO-GO
Concrete repository / #22B synthetic: MERGED（PR #110 / 62a43d7fbb5b33f69e0f4adfbba405ab00c1fb81）
Ready: YES（consumed）
Merge: DONE
実 SharePoint adapter / tenant integration: 別 Gate / NO-GO
...
```text
1. Decision-FC-3 を Accepted / Option C として正本化し living docs へ同期する
2. FC-1 / FC-2 Accepted 境界を維持する
