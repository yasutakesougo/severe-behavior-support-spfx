...
| **Decision-FC-4** | Catalog version identifier contract | **Accepted / Option C**。Complete identifier logical contract（一意性・opaque・edition対応・reuse禁止・fail-closed）。正本: [`decision-fc-4-catalog-version-identifier-contract.md`](./decision-fc-4-catalog-version-identifier-contract.md) | FC-3 Accepted（Option C） | UUID/hash/semver/DEC番号・schema・storage・provider・値一覧は決めない | materialization / schema / provider は別 Human Start。Implementation HOLD |
...
### C. FindingCode 業務カタログ（Decision-FC-1 / FC-2 / FC-3 / FC-4）
...
| version 文字列 / schema / storage / provider | FC-4 対象外 | **UNDECIDED / DO NOT START** |
| Implementation Start | FC-1 / FC-2 / FC-3 / FC-4 | **HOLD** |
...
11. Decision-OP-3    Observation period Schema / 制度 / 開放終端
12. Decision-RD-3    Review due 接近窓 / 算出 / 超過後
13. Decision-AS-EC-1 AssessmentSnapshot Entry Criteria（DEC-009 / GOV-AUD / Finding 境界後）
```
...
**Decision-FC-1 = Accepted / Option B**。**Decision-FC-2 = Accepted / Option C**。**Decision-FC-3 = Accepted / Option C**。**Decision-FC-4 = Accepted / Option C**。値一覧・enum・validator・version 文字列・storage / provider は未決。Issue #8 FindingCode DEC 番号は UNASSIGNED。
...
Dependency blocker（#29 mapping）: CLEARED
Concrete Repository Entry Review: PASS（Issue #22 comment 5224544473）
Next: SEV-2-VOCAB Canonical COMPLETE（DEC-018）。FC-1〜FC-4 Accepted / Option C系。FindingCode 値一覧未決。実 SharePoint adapter は別 Gate / NO-GO
Concrete repository / #22B synthetic: MERGED（PR #110 / 62a43d7fbb5b33f69e0f4adfbba405ab00c1fb81）
Ready: YES（consumed）
Merge: DONE
実 SharePoint adapter / tenant integration: 別 Gate / NO-GO
...
```text
1. Decision-FC-4 を Accepted / Option C として正本化し living docs へ同期する
2. FC-1 / FC-2 / FC-3 Accepted 境界を維持する
3. Implementation Start は HOLD を維持する（auto-start FORBIDDEN）
4. FindingCode 値一覧・identifier 物理方式・schema・storage・provider・実装には進まない
