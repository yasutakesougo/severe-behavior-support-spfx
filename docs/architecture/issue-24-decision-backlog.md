...
| 5 | 完全 Finding または findingIds 参照境界 | **未 / NOT MET**（read-only 監査 [`decision-as-ec-1-entry-5-6-7-read-only-canonicalization-audit.md`](./decision-as-ec-1-entry-5-6-7-read-only-canonicalization-audit.md)。FC/SEV 隣接 Accepted だけでは閉じない） |
| 6 | サービス別 `NOT_APPLICABLE` reason 正本または HOLD 方針 | **未 / NOT MET**（同監査。設計 HOLD 文言 ≠ Entry Acceptance） |
| 7 | Schema ID / schemaVersion / DTO versioning | **未 / NOT MET**（同監査。DEC-1 共通方針 ≠ AssessmentSnapshot Entry #7 MET） |
