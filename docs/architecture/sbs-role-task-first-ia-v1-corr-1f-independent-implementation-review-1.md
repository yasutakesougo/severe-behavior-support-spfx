# SBS-ROLE-TASK-FIRST-IA-V1 — CORR-1F Independent Implementation Review-1

Fresh Independent Implementation Review against Product PR exact HEAD only.
...
exact HEAD: 34cee752050017968c2c8bc80c1df591158e576b
...
P2: 3 (non-blocking)
REVIEW-CLEARED: NO
Human Task Acceptance: NOT ELIGIBLE
Ready / Merge / Deploy / LIVE WRITE: NOT AUTHORIZED
```

Blocking cause: exact-HEAD Contracts CI is red on Prettier format (`Verify contracts, skills, and scope` @ run `35068132252`). AC-1F-10 / Exact-head CI GREEN is not satisfied at `34cee752`.
...
| Product head SHA | review target | `34cee752050017968c2c8bc80c1df591158e576b` |
...
| Scope Review-1 | PASS / CONSUMED | present |
| AppShellChrome.tsx / primary-navigation.ts | unchanged | confirmed (diff name filter) |
| Product files outside Scope §3 | none | confirmed for Product runtime paths |
| Verification files vs Scope §6 | closed list | `.gitignore` extra → P2-2 |

Controlled input for this review = Product PR #616 head `34cee752…` + Scope §3–§8 / §16 + CI/smoke evidence at that tip.
...
| Exact-head reproducible evidence (AC-1F-10) | **FAIL** | format CI red at exact HEAD; see P1-1 |
...
| Exact-head CI GREEN | **FAIL** | Contracts format check FAILURE |
| Browser smoke 7/7 | **PASS** | workflow run `35068132231` SUCCESS; scenarios listed below |
| Unit `field-staff-task-navigation.test.ts` | **PASS** | SPFx build job: 7 passed, 0 failed |
| Human Task Acceptance | **NOT CLAIMED** | explicit |
| Ready / Merge / Deploy | **NOT AUTHORIZED** | explicit |

---

## AC-1F evidence map @ `34cee752`
...
| AC-1F-10 | **FAIL** | smoke PASS but Contracts format FAIL blocks exact-head GREEN | — |
| AC-1F-11 | PASS (invariant) | no PL/AA V1 Global claim | AppShellChrome unchanged |

Smoke scenarios observed PASS (CI artifact `evidence.json`, recorded merge-ref SHA `39fa058…` = merge of `34cee752` into `6b218125`; Product `spfx/` tree identical for practical binding):
...
|---|---|---|---|---|---|
| P1-1 | P1 | OPEN | Exact-head Contracts CI is red: `prettier --check` fails on 4 CORR-1F files. AC-1F-10 / Exact-head CI GREEN not met at `34cee752`. | CI run `35068132252` job `Verify contracts, skills, and scope` → Format check FAILURE. Local reproduce: same 4 paths warn. Files: `run-smoke.mjs`, `field-staff-task-navigation.ts`, `field-staff-task-navigation.test.ts`, `ScaffoldShell.tsx`. | Format-only correction on #616; re-bind Independent Implementation Review to new exact HEAD. |
...
| P2-3 | P2 | OPEN | Browser smoke `evidence.json` records `implementationHead=39fa058…` (PR merge ref from `github.sha`), not PR head `34cee752…`. | Artifact from run `35068132231`. | Non-blocking while Product tree matches; prefer binding `github.event.pull_request.head.sha` on next smoke revision. |

```text
P0: none
P1: P1-1 OPEN (format / exact-head CI)
P2: P2-1, P2-2, P2-3 OPEN
```
...
Human Task Acceptance PASS = not proven
```
...
```text
Independent Implementation Review-1 = FAIL / CORRECTION REQUIRED
Human Task Acceptance = NOT CLAIMED / NOT ELIGIBLE
Ready / Merge / Deploy / LIVE WRITE = NOT AUTHORIZED
SharePoint / M365 / Entra mutation = NOT AUTHORIZED
```

Implementer PR body claims Human Correction Implementation GO consumed for CORR-1F tranche. This review does not re-issue that GO and does not treat Review FAIL as GO revocation; it only withholds REVIEW-CLEARED until P1-1 is closed at a new exact HEAD.
...
```text
1. On PR #616: Prettier format correction for the 4 failing files only (no Product scope widen)
2. Push → new exact HEAD
3. Confirm Exact-head CI GREEN (Contracts format + existing SPFx build/smoke)
4. Fresh Independent Implementation Review-2 @ new HEAD
5. Only after Review PASS / REVIEW-CLEARED: Human Task Acceptance (separate Human gate)
```

```text
STOP = no Ready / Merge / Deploy / LIVE WRITE
