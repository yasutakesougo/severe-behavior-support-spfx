# AssessmentSnapshot — SharePoint Read/Write Conversion Contract

この文書は、**Decision-AS-CONVERSION-1** Accepted / LOCKED に基づく
AssessmentSnapshots CV-REQ（MAP-AS-001〜008）向け
**Read Conversion / Write Conversion 正本**である。

```text
repository: yasutakesougo/severe-behavior-support-spfx
Decision ID: Decision-AS-CONVERSION-1
Kind: Conversion contract（docs-only）
...
## 2. Global failure / conversion principles（LOCKED）
...
## 3. Topic rules（LOCKED）

### C-1 Required text（MAP-AS-001 / 005 / 008）— ACCEPTED C-1-A
...
### C-2 Choice（MAP-AS-002 / 003）— ACCEPTED C-2-DERIVED

NO NEW SEMANTIC DECISION（CHOICE-OPTIONS-1 + MF-1 から DERIVED）。
...
### C-3 reasonCodes JSON（MAP-AS-004）— ACCEPTED C-3-A
...
### C-4 DateOnly（MAP-AS-006 / 007）— ACCEPTED C-4-A
...
P2-002 OPEN / CARRY-FORWARD:
  SharePoint client DateOnly wire-form enumeration
  Decision blocker: NO
  Adapter impl must handle wire forms without changing the civil day
```
...
|---|---|---|---|---|---|---|---|---|
| MAP-AS-001 | snapshotId | snapshotId | 1行テキスト | Text → non-empty string pass-through；empty/ws/null/missing/non-string fail-closed | non-empty string → Text pass-through | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED | C-1-A；Acceptance；complete-contract；VR-1 |
| MAP-AS-002 | recordStatus | recordStatus | 選択肢 | stored Choice → enum；label unused；unknown/missing/null/empty fail-closed | enum → stored Choice | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED（DERIVED） | C-2-DERIVED；Acceptance；CHOICE-OPTIONS-1；VR-1 |
| MAP-AS-003 | result | result | 選択肢 | stored Choice → enum；label unused；unknown/missing/null/empty fail-closed | enum → stored Choice | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED（DERIVED） | C-2-DERIVED；Acceptance；CHOICE-OPTIONS-1；VR-1 |
| MAP-AS-004 | reasonCodes | reasonCodes | 複数行テキスト | Note JSON → unique string[]（isReasonCode；order exact；duplicates fail-closed）；invalid/null/missing fail-closed | unique string[] → compact JSON array in Note | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED | C-3-A；Acceptance；COLUMN-NAMES-1 Representation=JSON；complete-contract |
| MAP-AS-005 | ruleSetVersion | ruleSetVersion | 1行テキスト | same as MAP-AS-001 | same as MAP-AS-001 | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED | C-1-A；Acceptance；complete-contract；VR-1 |
| MAP-AS-006 | periodStart | periodStart | 日付のみ | DateOnly → `YYYY-MM-DD` civil date；no TZ rewrite | `YYYY-MM-DD` → DateOnly civil date | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED | C-4-A；Acceptance；contracts-v1；isValidIsoDate；VR-1 |
| MAP-AS-007 | periodEnd | periodEnd | 日付のみ | same as MAP-AS-006 | same as MAP-AS-006 | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED | C-4-A；Acceptance；contracts-v1；isValidIsoDate；VR-1 |
| MAP-AS-008 | inputFingerprint | inputFingerprint | 1行テキスト | same as MAP-AS-001 | same as MAP-AS-001 | RW-1 + MF-1 fail-closed | ACCEPTED / LOCKED | C-1-A；Acceptance；complete-contract；VR-1 |

```text
Decision Status legend（this document）:
  ACCEPTED / LOCKED = Human Acceptance of Decision-AS-CONVERSION-1
  DERIVED annotation = no new semantic decision beyond prior Accepted Choice contract
```
...
NOT claimed by this Acceptance:
  ...
  schema / DTO wiring
  SharePoint item round-trip verification as Implementation Start
  Deploy / real data GO
  P2-002 closure
```
...
MAP-AS-001〜008 conversion rows: ACCEPTED / LOCKED
mapping-complete: NOT YET
adapter / Implementation Start: HOLD（separate gates）
Next gate（PR process）: HUMAN READY DECISION FOR PR #207
```
