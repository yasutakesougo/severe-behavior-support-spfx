...
| FindingCode 業務カタログ | Decision-FC-1 Accepted / Option B（Issue #8 new DEC） | ownership COMPLETE。FC-2〜FC-4 Accepted / Option C。FC-5 CANDIDATE。値一覧未決 | **不可**（実装 HOLD） | Issue #8 FindingCode DEC 番号 UNASSIGNED。値一覧・UUID/hash/semver・schema / storage / provider は後続 |
...
| FindingCode カタログ Decision | 写像表・採番 | FC-1〜FC-4 Accepted。FC-5 representation ownership CANDIDATE。値一覧は後続。次単位へ値を入れない |
...
4. FindingCode 業務カタログ Decision — **FC-1〜FC-4 Accepted**。**FC-5 CANDIDATE**（identifier representation ownership）。実装 HOLD。値一覧未決
