...
| FindingCode 業務カタログ | Decision-FC-1 Accepted / Option B（Issue #8 new DEC） | ownership COMPLETE。FC-2 Accepted / Option C。FC-3 Accepted / Option C。値一覧未決 | **不可**（実装 HOLD） | Issue #8 FindingCode DEC 番号 UNASSIGNED。値一覧・version 文字列・schema / storage / provider は後続 |
...
| FindingCode カタログ Decision | 写像表・採番 | FC-1 ownership Accepted（Option B）。FC-2 delivery Accepted（Option C）。FC-3 logical contract Accepted（Option C）。値一覧は後続。次単位へ値を入れない |
...
4. FindingCode 業務カタログ Decision — **FC-1 Accepted / Option B**。**FC-2 Accepted / Option C**。**FC-3 Accepted / Option C**（complete logical contract surface）。実装 HOLD。値一覧未決
