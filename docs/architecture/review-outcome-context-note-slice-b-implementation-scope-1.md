...
status: SCOPE DEFINED / AWAITING INDEPENDENT SCOPE REVIEW
...
### S3 — Note normalization and maximum length

```text
Decision: max normalized note length = 255 characters
```
...
```text
input.trim()
all-whitespace => blank => null note record
1..255 normalized characters => valid note
>255 normalized characters => INVALID / no partial capture
```

Line breaks may remain plain text within the 255-character limit.
HTML is never interpreted; React text rendering only. `dangerouslySetInnerHTML`
is forbidden.
...
State remains keyed by the existing review-context key:
...
2. Read transient note draft from ReviewOutcomeCaptureView.
3. Normalize note text.
4. If normalized note >255 => INVALID; store nothing.
...
This is transient uncommitted input only.
It is not evidence, persistence, or readback state.
...
```text
{normalized-or-current-character-count} / 255
```

Over-limit fail-closed error:
...
### S12 — Exact Product file surface
...
over-limit note => no partial success
...
255-character boundary UX does not create horizontal overflow
pageerror = 0
...
INV-SB5  Max normalized note length is 255; over-limit fails closed.
INV-SB6  Decision + note capture is atomic; no partial success.
INV-SB7  After success, decision and note mutation controls are disabled.
INV-SB8  Note readback is secondary and plain text only.
INV-SB9  CHANGE_REQUIRED still does not imply N+1 exists.
INV-SB10 MonitoringVersion is not introduced.
INV-SB11 SharePoint / LIVE WRITE / Deploy are absent.
INV-SB12 AI cannot author or infer note content.
INV-SB13 Generic comments/timeline/history are not introduced.
INV-SB14 Slice A materials/identity/role cues remain intact.
```
...
```text
1. Re-read locked Definition + this exact Scope.
...
5. Extend ReviewOutcomeCaptureView note draft/input/readback.
...
SAC-B4  255-character normalization/limit is explicit and fail-closed.
SAC-B5  Atomic capture and no-partial-success behavior are fixed.
SAC-B6  Exact Product/domain and verification file surfaces are bounded.
SAC-B7  Existing OutcomeId mint path remains untouched.
SAC-B8  Post-success note/decision mutation remains forbidden.
SAC-B9  Rendered acceptance + Actual Staff gate remain downstream prerequisites.
SAC-B10 MonitoringVersion / N+1 / SharePoint / LIVE WRITE remain OUT.
SAC-B11 Human Implementation Start is still a separate gate.
```

## 9. Stop conditions

```text
Independent Scope Review required before Human Implementation Start GO.
If review finds P0/P1 => Scope Correction + exact re-review.
...
```text
Independent Scope Review-1 = NOT STARTED
