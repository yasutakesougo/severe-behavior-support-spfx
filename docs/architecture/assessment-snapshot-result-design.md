...
5. 完全なFinding契約またはfindingIds参照境界が確定済み（**未 / NOT MET** — 監査 [`decision-as-ec-1-entry-5-6-7-read-only-canonicalization-audit.md`](./decision-as-ec-1-entry-5-6-7-read-only-canonicalization-audit.md)）
6. サービス別NOT_APPLICABLE reason codeの正本またはHOLD方針が確定済み（**未 / NOT MET** — 同監査）
7. Schema ID・schemaVersion・DTO versioning方針が確定済み（**未 / NOT MET** — 同監査）
...
- Decision-AS-EC-1 overall Entry satisfied（Entry #5/#6/#7 **未 / NOT MET**。Entry #2 は PASS / MET。監査 [`decision-as-ec-1-entry-5-6-7-read-only-canonicalization-audit.md`](./decision-as-ec-1-entry-5-6-7-read-only-canonicalization-audit.md)）
