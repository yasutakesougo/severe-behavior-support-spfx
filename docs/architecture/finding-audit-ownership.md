...
Decision-FC-3（CANDIDATE / snapshot logical contract / NOT ACCEPTED）: docs/architecture/decision-fc-3-finding-code-catalog-snapshot-logical-contract.md
...
| DEC正本台帳 | Issue #8 | `DEC-001〜018`。FindingSeverity 不採用は **DEC-018**（comment `5225426738`）。VOCAB Canonical COMPLETE。FindingCode catalog ownership は Decision-FC-1 Accepted（Option B / 新 DEC・番号 UNASSIGNED）。delivery は Decision-FC-2 Accepted（Option C）。FC-3 snapshot logical contract は CANDIDATE。PURPOSE は MHLW-first RECORDED |
...
Decision-FC-3 は CANDIDATE / NOT ACCEPTED（snapshot logical contract）。
...
- FindingCode 業務カタログ ownership は Decision-FC-1 **Accepted / Option B**（Issue #8 new DEC / business DEC）。delivery は Decision-FC-2 **Accepted / Option C**。snapshot logical contract は Decision-FC-3 **CANDIDATE**。値一覧・version 文字列・schema / storage / provider・実装は開始しない（[`decision-fc-1-finding-code-catalog-ownership.md`](./decision-fc-1-finding-code-catalog-ownership.md)、[`decision-fc-2-finding-code-catalog-delivery-boundary.md`](./decision-fc-2-finding-code-catalog-delivery-boundary.md)、[`decision-fc-3-finding-code-catalog-snapshot-logical-contract.md`](./decision-fc-3-finding-code-catalog-snapshot-logical-contract.md)）
