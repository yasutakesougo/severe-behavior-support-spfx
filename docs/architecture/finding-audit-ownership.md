...
| `GOV-RULE-01` | 行動関連点数の評価周期 | Issue #19（**Accepted / LOCKED** — アプリ独自固定周期なし；正式認定・更新・変更認定の有効期間が正本。3年は基本値（ハードコード禁止）。正本 [`decision-gov-rule-01-evaluation-cycle-acceptance.md`](./decision-gov-rule-01-evaluation-cycle-acceptance.md)） |
...
| `GOV-RULE-04` | 必要観察件数 | Issue #19（**Accepted / LOCKED** — minimumObservationCount = NOT FIXED。前回〜今回；独立事象・ケースを1件。正本 [`decision-gov-rule-04-observation-count-acceptance.md`](./decision-gov-rule-04-observation-count-acceptance.md)） |
