...
Decision-FC-6（CANDIDATE / businessOwnershipRef logical contract / NOT ACCEPTED / Selected NONE）: docs/architecture/decision-fc-6-business-ownership-ref-logical-contract.md
...
| DEC正本台帳 | Issue #8 | `DEC-001〜018`。FindingSeverity 不採用は **DEC-018**（comment `5225426738`）。VOCAB Canonical COMPLETE。FindingCode catalog ownership は Decision-FC-1 Accepted（Option B / 新 DEC・番号 UNASSIGNED）。delivery は Decision-FC-2 Accepted（Option C）。FC-3 snapshot logical contract は Accepted / Option C。FC-4 catalog version identifier は Accepted / Option C。FC-5 representation ownership は Accepted / Option C。FC-6 businessOwnershipRef は CANDIDATE。PURPOSE は MHLW-first RECORDED |
...
Decision-FC-6 は CANDIDATE / NOT ACCEPTED（businessOwnershipRef logical contract / Selected NONE）。
