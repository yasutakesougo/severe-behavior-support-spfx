...
Fifteenth residual Decision: SELECTED / A — DEC-6 concrete mapping（selection only；Decision-AS-DEC6-MAPPING-1 compare OPEN）
FindingCode: HOLD
A-5: HOLD
...
DEC-6 concrete mapping: SELECTED FOR NEXT DECISION（compare OPEN / NOT ACCEPTED）
...
| **DEC-6** | SharePoint 列変換 | 技術/adapter | adapter Entry 前提 | 技術契約寄り | N/A | **Fifteenth residual SELECTED** — Decision-AS-DEC6-MAPPING-1 compare OPEN / NOT ACCEPTED（[`decision-assessment-snapshot-dec6-mapping-packet.md`](./decision-assessment-snapshot-dec6-mapping-packet.md)）。具体 Site/List/Internal Name = NOT DECIDED | **D** 候補（業務ルール発明ではない）／compare OPEN |
...
Order:
 ...
 16. Fifteenth residual: DEC-6 concrete mapping（SELECTED / A；concrete values HOLD / NOT DECIDED）
     compare packet: decision-assessment-snapshot-dec6-mapping-packet.md（OPEN / NOT ACCEPTED）
...
DEC-6 concrete mapping: SELECTED / compare OPEN / values NOT DECIDED
Post-retention deletion: OPEN / AUTO-START FORBIDDEN
```
