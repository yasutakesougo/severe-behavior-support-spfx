...
Do NOT:
  ...
  Auto-start / auto-Accept GOV-AUD-05
  Re-decide Accepted Decisions（含む Decision-RD-3）
  Invent FindingCode / days / roles / approvers / notice rules
  Implementation Start
```

## 7. Next after Human Policy FINAL CONSISTENT / RD-3 Accepted

```text
Order:
  1. Human Policy Accepted / FINAL CONSISTENT（DONE / PR #151）
  2. First residual Decision: Decision-RD-3 Accepted / LOCKED（DONE）
  3. 他残存 Decision を一件ずつ Human 選定・判定
