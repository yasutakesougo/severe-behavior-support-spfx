...
This is the first repository-visible exact Implementation Scope document for this correction cycle.
...
status: COMPLETE / AWAITING FRESH INDEPENDENT IMPLEMENTATION SCOPE RE-REVIEW
date: 2026-09-20
mode: DEFINITION / DOCS ONLY
Product / test / smoke mutation by this document: NONE

current main: 35cf211aaef9d30975775b531947ac93283d6d5e

Locked Definition Basis (parent; PRESERVED; Modification = NONE):
...
Independent Scope Re-Review PASS ≠ Human Implementation Start GO ≠ Ready ≠ Merge.

---

## 0. Draft-1 provenance (normative for this correction cycle)

```text
Prior described Draft-1
  = unavailable as repository evidence
  = not locatable in repository / branch / commit / worktree / snapshot / GitHub
  = NOT REVIEWABLE
  = NOT A VALID REVIEW BASIS
  = superseded for review purposes by Draft-2
```
...
Draft-2 is the sole target of the next Fresh Independent Implementation Scope Re-Review.

This is a scope-document correction only.

```text
Locked Definition = PRESERVED
Definition Modification = NONE
```
...
Verified on `main` @ `35cf211aaef9d30975775b531947ac93283d6d5e` before writing this Draft-2.

| Basis | Exact repository path | Current blob |
|---|---|---|
| Current main | `origin/main` | commit `35cf211aaef9d30975775b531947ac93283d6d5e` |
...
Completed Fresh Independent Implementation Scope Review:
...
That review’s findings F-P0-1 and F-P1-1..F-P1-5 are closed or addressed in this body. The review itself is not reconstructed as a backdated repository artifact. Draft-2 is the reviewable scope.

P2 = 2 remains OPEN / non-blocking. Exact P2 texts were not repository-visible on current main. This Draft-2 does not invent or close them. Deferred OUT items in §8 keep those leftovers from being smuggled closed.

---

## 3. Scout summary (main @ 35cf211aaef9d30975775b531947ac93283d6d5e)

### 3.1 Observed Product state (read-only)

| Area | Exact path | Observation | This-unit impact |
|---|---|---|---|
| Presentation vocabulary | `spfx/src/shell/ux/presentation-role.ts` | Defines `FIELD_STAFF` / `PLANNER` / `ADMIN_AUDIT` synthetic roles, hints, Overview/UserDetail/SupportPlan order helpers. No ADMIN_AUDIT Global / D-* destination contract. | **Reuse read-only.** Destination/navigation ownership MUST NOT be added here (F-P1-1). |
...
| App shell chrome | `spfx/src/shell/ux/AppShellChrome.tsx` | Unconditionally renders `SHELL_PRIMARY_NAV_ITEMS` as Global nav for **all** roles. `onPresentationRoleChange` notifies parent only for `FIELD_STAFF \| PLANNER`. ADMIN_AUDIT stays chrome-local (FE-F001 leftover). No dedicated AppShellChrome test file exists. | **IN.** ADMIN_AUDIT-only not-render of legacy Global. Parent notify must include ADMIN_AUDIT. |
| FIELD_STAFF Task-First | `spfx/src/shell/ux/field-staff-task-navigation.ts` + `.test.ts` | Dedicated navigation contract. `D-HOME` identity = `D-TODAY` via `FIELD_STAFF_HOME_DESTINATION = "D-TODAY"`. | Regression-only. OUT of rewrite. |
| PLANNER Task-First | `spfx/src/shell/ux/planner-task-navigation.ts` + `.test.ts` | Dedicated navigation contract. Distinct `D-HOME`. | Regression-only. OUT of rewrite. Pattern source for ADMIN_AUDIT module. |
| ADMIN_AUDIT Task-First module | *(absent)* | No `admin-audit-task-navigation.ts`. | **New authorized module.** |
| Product entry | `spfx/src/webparts/scaffoldShellWebPart/components/ScaffoldShell.tsx` | `ScaffoldShellState = FieldStaffShellState \| PlannerShellState` only. `createInitialState()`: `PLANNER` branch, else FIELD_STAFF. ADMIN_AUDIT injection falls through to FIELD_STAFF Task-First. No dedicated ScaffoldShell test file exists. | **IN.** Explicit `AdminAuditShellState` + ADMIN_AUDIT render branch. |
| Task-First styles | `spfx/src/webparts/scaffoldShellWebPart/components/ScaffoldShell.module.scss` | CSS-hides SHELL-UX-7 primary-navigation for `FIELD_STAFF` and `PLANNER` only. ADMIN_AUDIT is not in that selector, so legacy Global remains visible for ADMIN_AUDIT. | **IN**, ADMIN_AUDIT-scoped only. CSS hide MUST NOT be the ADMIN_AUDIT uniqueness mechanism. |
| Export barrel | `spfx/src/shell/ux/index.ts` | Exports FIELD_STAFF + PLANNER task-navigation. No ADMIN_AUDIT task-navigation export. | **IN**, export wiring only. |
...
| Existing ADMIN smoke | `spfx/smoke/admin-demo-ux-polish-1/run-smoke.mjs` | ADMIN-DEMO-UX-POLISH-1 synthetic polish smoke. Not ADMIN-AUDIT-TASK-FIRST-V1 evidence. | Insufficient. Dedicated smoke required (F-P1-4). Do not repurpose. |
| FIELD_STAFF smoke | `spfx/smoke/sbs-role-task-first-ia-1/` | Labels / order / CSS `display:none` uniqueness / 390×844. | Regression evidence for FIELD_STAFF. Do not rewrite. |
| PLANNER smoke | `spfx/smoke/sbs-planner-top-level-ia-v1/` | Labels / order / CSS `display:none` uniqueness. | Regression evidence for PLANNER. Do not rewrite. |
| Sibling smoke CI | `.github/workflows/sbs-planner-top-level-ia-v1-browser-smoke.yml` | Exact PR head checkout, Noto CJK fonts, isolated puppeteer/esbuild/sass. | Pattern for dedicated ADMIN_AUDIT smoke workflow. |
...
## 4. Finding closure (this Draft-2)

| Finding | Required correction | Draft-2 close |
|---|---|---|
| **F-P0-1** Draft-1 absent / not reviewable | Create Draft-2 as exact repository-visible docs-only scope; make it the sole Re-Review target; enumerate authorized file surface, acceptance criteria, verification commands | §0, this body, §7, §9, §10 |
| **F-P1-1** ADMIN_AUDIT navigation contract ownership | Dedicated `admin-audit-task-navigation.ts` following FIELD_STAFF / PLANNER. Do not place destination semantics in `presentation-role.ts`. | §5.1, §6, §7.1 |
| **F-P1-2** legacy Global suppression | ADMIN_AUDIT-only: legacy Global **NOT RENDERED**. Task-First Global = single active Global. FIELD_STAFF / PLANNER regression unchanged. FE-F003 uniqueness directly testable. | §5.2, §7.2, AC-AA-TF-13/14/15/16 |
| **F-P1-3** ScaffoldShell ADMIN_AUDIT state | Explicit `AdminAuditShellState`. First paint `D-OPS`. `D-HOME == D-OPS` literal identity. Parent preserves first paint / navigation / role switch / restoration / deep link / fail-closed. | §5.3, §6.2, §6.4, AC-AA-TF-6..12 |
| **F-P1-4** dedicated ADMIN_AUDIT smoke | Dedicated smoke surface. Smoke PASS ≠ AA-HTA PASS. No FS/PL regression claim beyond gathered evidence. | §7.3, §10, AC-AA-TF-21/22/23 |
| **F-P1-5** acceptance expansion | Replace insufficient S1–S12. Every listed requirement independently verifiable. | §9 (AC-AA-TF-1..23) + §10 mapping |

S1–S12 from the unavailable Draft-1 are **not restated** and are **not a review basis**. AC-AA-TF-1..23 is the acceptance set.

---

## 5. Architectural ownership (normative)

### 5.1 Dedicated ADMIN_AUDIT navigation module (F-P1-1)
...
### 5.2 ADMIN_AUDIT-only legacy Global suppression (F-P1-2)
...
### 5.3 ScaffoldShell ADMIN_AUDIT state ownership (F-P1-3)
...
`ScaffoldShell.module.scss` may:

```text
IN  reuse existing Task-First classes for the ADMIN_AUDIT layer
IN  ADMIN_AUDIT-scoped chrome hides that do not substitute for AppShellChrome not-render
OUT using display:none (or equivalent) of [data-shell-ux="primary-navigation"]
    as the ADMIN_AUDIT uniqueness mechanism
OUT changing FIELD_STAFF / PLANNER selectors in a way that alters their current behavior
```
...
domain / schema / persistence / SharePoint adapters
...
If a file not listed in §7.1–§7.3 is required to satisfy AC-AA-TF-1..23, HOLD (H-3 / H-4). Do not expand silently.
...
S1–S12 are not used. Every requirement below is independently verifiable.

| ID | Criterion | Locked / finding trace |
|---|---|---|
| AC-AA-TF-1 | ADMIN_AUDIT Task-First Global labels are exactly `運用確認`, `証跡`, `探す`. | CORR-2A §2.4; F-P1-5.1 |
| AC-AA-TF-2 | Exact Global ordering is `運用確認 · 証跡 · 探す`. | CORR-2A §2.4 / C5 §7.2; F-P1-5.2 |
| AC-AA-TF-3 | Global 運用確認 navigates to Destination identity `D-OPS` only. | CORR-2A §2.4; F-P1-5.3 |
| AC-AA-TF-4 | Global 証跡 navigates to Destination identity `D-EVIDENCE` only. | CORR-2A §2.4; F-P1-5.4 |
| AC-AA-TF-5 | Global 探す navigates to Destination identity `D-FIND-PERSON` only; not `D-FIND-RECORD`. | CORR-2A §2.4 / §2.5; F-P1-5.5 |
| AC-AA-TF-6 | ADMIN_AUDIT first paint after usable session = `D-OPS` / C6 `運用確認`. | CORR-2B §3.4 / C5 §7.1; F-P1-5.6 |
| AC-AA-TF-7 | `D-HOME == D-OPS` is literal identity. No separate Home place. Restore token `D-HOME` is the same identity as `D-OPS`. | CORR-2B §3.4 / C6; F-P1-3; F-P1-5.7 |
| AC-AA-TF-8 | Role switch **into** ADMIN_AUDIT notifies the parent shell and lands ADMIN_AUDIT Task-First at `D-OPS`. Chrome-local leftover (FE-F001) is closed. | C6; FE-F001; F-P1-3; F-P1-5.8 |
| AC-AA-TF-9 | Role switch **away from** ADMIN_AUDIT restores FIELD_STAFF or PLANNER existing Task-First behavior and does not keep ADMIN_AUDIT Global as a second nav. | C6; F-P1-3; F-P1-5.9 |
| AC-AA-TF-10 | Destination state restoration works for supported ADMIN_AUDIT Destinations in this slice (`D-OPS` / `D-EVIDENCE` / `D-FIND-PERSON`). | C6; F-P1-3; F-P1-5.10 |
| AC-AA-TF-11 | Deep-link restoration of role + Destination + object succeeds where this slice supports it (`ADMIN_AUDIT` + Destinations in AC-AA-TF-10; object not required for these Global Destinations). | C6; F-P1-5.11 |
| AC-AA-TF-12 | Unsupported or invalid restoration fail-closes. It does not mint the requested place and does not guess `D-OPS` as a successful restore of that place. | C6; F-P1-3; F-P1-5.12 |
| AC-AA-TF-13 | When effective presentation role is ADMIN_AUDIT, legacy Global `概要 / 利用者 / 記録` is **not rendered**. | CORR-1A; FE-F003; F-P1-2; F-P1-5.13 |
| AC-AA-TF-14 | ADMIN_AUDIT Task-First Global is the single active Global navigation (FE-F003 uniqueness directly testable). | CORR-1A; F-P1-2; F-P1-5.14 |
| AC-AA-TF-15 | FIELD_STAFF existing navigation behavior is preserved (labels, order, CSS-hide uniqueness, destinations). | CORR-1F/1G; F-P1-2; F-P1-5.15 |
| AC-AA-TF-16 | PLANNER existing navigation behavior is preserved (labels, order, Distinct D-HOME). | PLANNER Top-Level IA; F-P1-2; F-P1-5.16 |
| AC-AA-TF-17 | Presentation role remains distinct from authorization role. No Entra / DEC-014 / auth-role invention. | Packet §4; F-P1-5.17 |
| AC-AA-TF-18 | `D-OPS` does not grant approval authority. | Packet §4 / C4; F-P1-5.18 |
| AC-AA-TF-19 | `D-EVIDENCE` does not imply evidence acceptance. | Packet C4; F-P1-5.19 |
| AC-AA-TF-20 | Task-First UI does not grant publish / deploy / delete / LIVE WRITE authority. | Packet §4 / C10; F-P1-5.20 |
| AC-AA-TF-21 | Automated smoke does not consume AA-HTA. Smoke PASS ≠ AA-HTA PASS. | Packet C9; F-P1-4; F-P1-5.21 |
| AC-AA-TF-22 | 390×844 ADMIN_AUDIT Task-First rendered evidence exists. | F-P1-4; F-P1-5.22 |
| AC-AA-TF-23 | Desktop ADMIN_AUDIT Task-First rendered evidence exists. | F-P1-4; F-P1-5.23 |
...
# current main equals 72ee81ec92aea6c1558f25232961259504affab9
```

### 10.2 Unit / typecheck / lint / scope
...
# SPFx Heft (authorized tests include §7.2 new files + existing FS/PL tests)
cd spfx
npx heft test --clean
```

### 10.3 Dedicated ADMIN_AUDIT Task-First smoke
...
| AC-AA-TF-26 | Heft test PASS at exact HEAD | smoke `implementationHead` recorded |
...
| H-6 | Destination / Global contract is added to `presentation-role.ts` | Stop; violates F-P1-1 |
| H-7 | ADMIN_AUDIT uniqueness is implemented only as CSS hide of still-rendered legacy Global | Stop; violates F-P1-2 NOT RENDERED |
| H-8 | `D-HOME == D-OPS` is implemented as a resolver / second place | Stop; violates F-P1-3 literal identity |
...
| H-13 | Draft-1 is cited as review basis | Stop; §0 provenance |

---

## 12. Fresh Independent Implementation Scope Re-Review questions

Reviewers must use **this Draft-2 body only**. Prior described Draft-1 is not a review basis.

| ID | Question |
|---|---|
| Q1 | Is Draft-2 repository-visible and the sole Re-Review target, without reconstructing Draft-1? |
...
| Q8 | Do AC-AA-TF-1..23 cover every F-P1-5 requirement without leaving switching / default / restoration / uniqueness / fail-closed implicit? |
| Q9 | Are authorized files closed under §7, and are FIELD_STAFF / PLANNER / `presentation-role.ts` / `primary-navigation.ts` OUT of destination-contract mutation? |
...
```text
PASS CONDITION for later Independent Scope Re-Review
...
A Scope Review PASS does not itself consume Human Implementation Start GO.
...
Completed Fresh Independent Implementation Scope Review = HOLD / CONSUMED as correction input
Implementation Scope Definition Draft-2 (this document) = COMPLETE
Independent Scope Re-Review against Draft-2 = NOT YET
...
```text
ALLOWED NEXT:
  Fresh Independent Implementation Scope Re-Review against this Draft-2 body only
...
| Current main (scout) | `35cf211aaef9d30975775b531947ac93283d6d5e` |
...
| Draft-1 | unavailable as repository evidence; superseded by Draft-2 |
