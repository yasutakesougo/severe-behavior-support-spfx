...
AUTONOMY-POLICY-V1（AI Development OS レーン / autonomy policy contract）: `docs/process/autonomy-policy-v1.md`（AUTO-1-A = ACCEPTED / LOCKED；Authorization expansion NONE；Implementation Start NOT GRANTED；AUTO-2/3/4・Cursor execution backend NOT STARTED；LOW-AUTO-PILOT-V2 NOT AUTHORIZED；capability 分類 ≠ 有効化；`pull_request.merge` = HUMAN_ONLY かつ Gateway 実装 MUST NOT EXIST）
...
| AUTONOMY-POLICY-V1（AUTO-1-A ACCEPTED / LOCKED / enablement NOT GRANTED） | `docs/process/autonomy-policy-v1.md` |
...
- AUTONOMY-POLICY-V1 の capability 実行有効化・Registry / Gateway / backend 実装開始（`docs/process/autonomy-policy-v1.md` AUTO-1-A ACCEPTED / LOCKED でも Authorization expansion = NONE；別 Human Explicit GO と上位正本整合が必要）
