...
AUTO-1 AUTONOMY-POLICY-V1（AI Development OS 自律実行ポリシー契約）: `docs/process/autonomy-policy-v1.md`（AP1-A = ACCEPT / ACCEPTED / LOCKED；Implementation Start = NOT GRANTED；Authorization effect NONE；Gateway に `pull_request.merge` を持たない；LOW-AUTO-PILOT-V2 NOT AUTHORIZED；AssessmentSnapshot EC-3/EC-4 は別レーンで非スキップ；machine-readable = `docs/process/autonomy-policy-v1.json`）
...
| AUTO-1 AUTONOMY-POLICY-V1（AP1-A ACCEPTED / LOCKED / Start NOT GRANTED） | `docs/process/autonomy-policy-v1.md` |
...
- AUTO-1 AUTONOMY-POLICY-V1 の Implementation Start / Capability Registry / Action Gateway / Cursor execution backend 実装（`docs/process/autonomy-policy-v1.md` AP1-A ACCEPTED / LOCKED でも Implementation Start = NOT GRANTED；別 Implementation GO が必要）
- AUTO-1 による LOW-AUTO-PILOT-V2 enablement / Ready auto / Merge auto（NOT AUTHORIZED）
...
- Action Gateway への `pull_request.merge` / SharePoint schema write 等 FORBIDDEN capability の搭載、および Decision accept 等 HUMAN_ONLY capability の auto 露出
- AUTO-1 recording PR の Ready / Merge（Acceptance 単独では付与しない；Human Ready / Merge Decision が必要）
