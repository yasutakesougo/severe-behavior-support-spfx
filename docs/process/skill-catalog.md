...
- 検証スクリプト: `scripts/verify-skills.mjs` / `scripts/verify-ui-catalog.mjs` / `scripts/verify-ui-templates.mjs` / `scripts/ui-sem/run.mjs`
- 実行コマンド: `npm run verify:skills` / `npm run verify:ui-catalog` / `npm run verify:ui-templates` / `npm run lint:ui-sem`
...
- CI（verify:skills / verify:ui-catalog / verify:ui-templates / lint:ui-sem / typecheck / tests / contracts-boundaries / scope check）
